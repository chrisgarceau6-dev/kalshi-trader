...
|---|---|---|
| `FLAT_BET_DOLLARS` | **45** | 50->25 in #151 (2026-08-22); 25->35 on 2026-08-28 and **35->50 on 2026-09-01, both OVERRIDES of that row's own "not on a good week" bar** — see PART II; **50->35 in #242 (2026-09-04), mechanical not discretionary**; **35->45 on 2026-09-10, THIRD override of the "not on a good week" bar — though that leg is itself satisfied for the first time, both 09-09 and 09-10 being losing days. Per the standing note a third override means the clause should be DELETED rather than overridden again; that deletion is raised separately rather than bundled into the commit it would have blocked**, on the ratio rule `bet <= (cash - STOP_BALANCE)/33` after cash fell to $1,643.65. Two overrides of the same clause in five days; a third means deleting the clause, not overriding it. **THIS ROW READ $50 FROM 2026-09-04 TO 2026-09-10 while the code read 35 — the same defect the CHARTER opens with ("config table said the bet was $50; it has been $25 since #151"), recurring within two weeks of the audit that was meant to end it. #242 shipped the constant and every derived figure but not this table. A sizing commit is not done until PART I is edited in the same PR.** `grep -n '^FLAT_BET_DOLLARS' late_certainty_trader.py` |
...
| book depth | **72** = `max(ceil(contracts x 1.5), 25)` at $45 | dynamic since #166; the legacy `MIN_BOOK_DEPTH=60` is no longer a gate. Not in harness (needs live book). **56 -> 80 on 2026-09-01 and 80 -> 56 again on #242 (2026-09-04), both purely from the bet change — nobody chose either move. The Sep 1 warning that this is the one self-rescaling constant that can COST volume is now moot in that direction: the requirement is back where it was before the raise.** **56 -> 72 on 2026-09-10 with the raise to $45. Cost MEASURED, not assumed: of 1,067 signals clearing depth 56 over 09-06..09-10, only 13 (1.2%) fall in the [56,72) band this newly rejects — `data/gatelog/*.csv`, columns `depth` and `min_depth`.** Reproduce: `python3 -c "import math; print(max(math.ceil(48*1.5),25))"` (48 contracts at $45 / 93c) |
...
| `STOP_BALANCE` | **400** | emergency floor. **Now 1.84x worst drawdown at $45, above the 1.5x target** — 40.6 losses on a $2,224.82 balance (2026-09-10 13:36 ET). **THE RATIO RULE IS EVALUATED ON TODAY'S CASH AND CANNOT SEE ONE STEP AHEAD: a repeat of the 22-loss worst drawdown at $45 takes cash to ~$1,274 and headroom to 0.88x, i.e. a FORCED CUT. The size that still clears 1.5x after such a run is $33.72 — that is where $35 came from, and Sep 1 -> Sep 4 -> #242 already ran this loop once at $50. Sizing at the ceiling does not merely risk a drawdown, it guarantees the cut that follows one.** History of the ratio: 1.32x at $35/$1,405 (Aug 28) -> 1.45x at $50/~$2,000 (Sep 1) -> 1.13x at $50/$1,643.65, which triggered #242 -> 1.62x at $35 on that same cash -> 2.37x today as the balance recovered. **The ratio rule cuts AND permits: the ceiling `(cash - STOP_BALANCE)/33` is $55.30 today, so $45 sits 19% below what the rule allows. Raising into that gap is a live decision for Chris and was taken as one on 2026-09-10; the post-drawdown column above is the part the ceiling does not price.** Growth remains **P&L, not a deposit** (confirmed 2026-09-01), so the revisit trigger has not fired and 400 stands. The floor is now 18.0% of balance vs the ~33% it was set at — designed behaviour under a growing account, but a progressively weaker brake. Re-check the RATIO on any deposit/withdrawal. `python3 scripts/kstat.py` reports headroom and ratio directly |
| `CONSEC_LOSS_LIMIT` | 9 | never fires in 68d either way |
| trailing-24h limit | **bet x 20** ($900 at $45) | emergency brake; denominated in bets so a sizing change cannot disarm it — it rescaled $1,000 -> $700 through #242 with no code change, which is the whole point of the bets denomination. Confirm against a live run: `gh run view --log <id> \| grep daily_loss_limit` prints the live figure. Now 40% of balance at $900, down from the 50% it held across the $35 and $50 changes, because the bet fell while the bankroll rose |
...
|---|---|---|
| **POST-LOSS COOLDOWN — PRE-REGISTERED 2026-09-03, NOT DEPLOYED. Code is in and `POST_LOSS_COOLDOWN_ENABLED = False`. Read the decision rule before enabling it** | Sep 3 | **Claim:** volatility clusters, so a signal entered shortly after a loss SETTLES is drawn from a worse regime than the 90-93c ask implies. Rule: block new entries for 15 minutes after any loss. **Trigger is ONE loss, and that is a mechanism decision, not a tuned one.** A higher trigger fires after the damage: on 2026-09-03 three losses cost -$146.23, but two settled SIMULTANEOUSLY at 17:45 and were both entered ~17:38, before the 17:30 loss had even settled — "halt after 2" could not have stopped the third. That is the same correlated-closes problem that took `CONSEC_LOSS_LIMIT` from 5 to 9, and it is why that constant is the wrong tool here: an emergency brake at 9 and a regime filter at 1 are different jobs. **EVIDENCE, measured on 3,128 real settlements (Jul 27 - Sep 3), and it is WEAK — read all of it before enabling.** (1) *The conditional effect is real and monotonic.* WR of the next entry by minutes since the last settled loss: **0-15m n=311 WR 69.1% ($-1.21/tr)**, 15-30m 88.3% (+$0.58), 30-60m 91.6% (+$0.96), 60-120m 91.9% (+$0.23), 120m+ 92.8% (+$0.20). Monotonic across five buckets on a 3,128-trade sample is not a fluke of bucketing. (2) *Only the mechanism-derived rule survives out-of-sample.* A sweep of 20 halt rules (N in 1-5 losses x 15/30/60/120 min) put **13 positive in-sample; OOS mean -$90 and only 3/13 still positive**, and the best in-sample rule (+$724) went **-$422 OOS**. The P&L deltas are also non-monotonic in cooldown length, which is a noise signature. **1L/15m, chosen from the conditional table rather than by P&L search, is positive in every window: full +$262, first half +$196, second half +$69, clean +$86.** Stable sign is the only thing separating it from the overfit family. (3) **AND IT IS NOT ESTABLISHED. Day-cluster bootstrap on the clean sample: +$54, 95% CI [-$180, +$329], P(>0)=0.640** — a coin flip. (4) **Most of the raw effect is the Jul 27-31 broken build**, where blocked trades won 45.4%. In clean recent data (Aug 21 - Sep 3) blocked WR is **91.9% against a ~91.8% break-even, i.e. edge ~ZERO**. Quoting the +$262 full-sample number without this sentence would repeat the error the Jul 27-31 bullet exists to stop. **POWER, stated up front so a null is not later mistaken for bad luck:** at n=200 blocked the SE on blocked WR is ~2.1pp, so this can only detect roughly a 4pp deficit; the clean-data point estimate is ~0pp. **The most likely outcome is a null, and the rule below is written so that a null means REMOVE IT.** **PRE-REGISTERED DECISION RULE — fixed 2026-09-03 BEFORE the flag was ever set true. Editing these after seeing results is the optional-stopping error #152 died of; a change requires a NEW dated row.** This is a filter, so **Invariant 8 applies at full force: the default is REVERT and the burden is on the filter to earn its place.** (1) **KEEP only if, at n>=200 blocked scoreable signals, blocked WR is BELOW its own break-even** (mean ask of the blocked set + 0.539pp fees) — i.e. it demonstrably avoided negative-edge trades. (2) **REVERT if blocked WR >= its own break-even at n>=200** — it is blocking winners. Same shape as z-gate rule 1. (3) **REVERT ON FUTILITY: if at n>=200 the blocked-edge cluster CI includes zero, that is NOT a pass.** A filter that cannot show negative edge on what it blocks does not stay, however pretty the point estimate. This clause exists because the point estimate today is positive and the CI already includes zero. (4) **REVERT if the block rate sits outside 1-15% for 3 consecutive days.** Expected ~4-6%, but note the denominators differ: the simulation blocked 164 of 2,835 SETTLED trades (5.8%), whereas live this is blocked signals over signals REACHING the gate. The band is deliberately wide because of that mismatch — it is a breakage detector, not a validation, and narrowing it later requires a new dated row. **HORIZON: ~4.3 blocked/day, so n=200 lands in ~46 days, i.e. mid-October. Do NOT read rules 1-3 before then, and do not stop early on a good stretch — or a bad one.** **DEPLOY PRECONDITION, and it is not optional: do NOT enable until the z-gate revert test resolves at n>=200 rejected (~Sep 7).** 63% of the z-gate's backtested value was sitting out whole settlements in whipsaw regimes — the SAME effect this rule targets — so running both at once confounds both measurements and neither could be attributed. **Enable:** `POST_LOSS_COOLDOWN_ENABLED = True`, nothing else. **Revert:** set it back to `False`, nothing else. **Implementation notes.** Reads `last_loss_ts`, which the settlement handler already maintains, so no new state and nothing to drift. **FAILS OPEN** on a missing, zero, unparseable or FUTURE timestamp — the future case is real, since `last_loss_ts` is stamped when the bot OBSERVES a settlement and Kalshi has stalled settlement 4h+ (2026-08-28). It **logs rather than halting the cycle**, emitting `[COOLDOWN-SKIP]` and `[COOLDOWN-PASS]` with ask and secs on BOTH sides: a cycle-level halt would stop the scan and destroy the record of what it skipped, which is the #233 blindness failure in a new place, and logging ask on both sides lets a monitor compute a REAL break-even for each group instead of assuming 92c. That format is fixed now, before any data exists, so it cannot later split the dataset. Pinned by 5 tests in `PostLossCooldownTests`, including a tripwire asserting the flag ships false. |
