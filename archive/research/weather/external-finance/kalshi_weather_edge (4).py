...
def cmd_poly_copy_backtest(a):
    ...
    n_before = len(rt)
    if a.filter_topics:
        keep = set(t.strip() for t in a.filter_topics.split(","))
        rt = rt[rt.topic.isin(keep)]
        print(f"topic filter [{','.join(keep)}]: {n_before} -> {len(rt)}")
    if a.filter_price_min > 0 or a.filter_price_max < 1:
        n0 = len(rt)
        rt = rt[(rt.entry_price >= a.filter_price_min) & (rt.entry_price <= a.filter_price_max)]
        print(f"price filter [{a.filter_price_min}-{a.filter_price_max}]: {n0} -> {len(rt)}")
    print(f"\n{len(rt)} round-trips after filters, {rt['wallet_name'].nunique()} wallets, "
...
def cmd_poly_arb_scan(a):
    """Multi-outcome arb detector. Scans events with N candidate markets (elections,
    tournaments) where each outcome resolves independently but exactly ONE wins.
    If sum(YES ask prices) > $1.00, then buying NO on EVERY candidate locks a
    guaranteed profit at resolution — retail's favorite-longshot bias creates these."""
    import pandas as pd
    GAMMA = "https://gamma-api.polymarket.com"
    CLOB = "https://clob.polymarket.com"

    print(f"fetching multi-outcome events with volume >= ${a.min_volume:,.0f}...")
    events, offset = [], 0
    while offset < 1000:
        try:
            ...
            print(f"gamma error: {e}"); break
        if not r.ok:
            print(f"gamma {r.status_code}: {r.text[:150]}"); break
        batch = r.json() or []
        if not batch:
            break
        events.extend(batch); offset += 100
        time.sleep(SLEEP)
        if len(batch) < 100:
            break
    print(f"got {len(events)} active events; filtering to multi-outcome...")

    multi = []
    for ev in events:
        markets = ev.get("markets") or []
        if len(markets) < a.min_candidates:
            continue
        vol = sum(float(m.get("volumeNum") or 0) for m in markets)
        if vol < a.min_volume:
            continue
        multi.append({
            "event_id": ev.get("id"),
            "event_title": (ev.get("title") or "")[:70],
            "n_markets": len(markets),
            "total_volume": vol,
            "markets": markets,
        })
    print(f"{len(multi)} multi-outcome events with >= {a.min_candidates} candidates "
          f"and total vol >= ${a.min_volume:,.0f}")
    if not multi:
        print("no events. Try lower --min-candidates or --min-volume."); return

    print(f"\nfetching current YES-side asks for each candidate market ({sum(e['n_markets'] for e in multi)} markets total)...")
    opportunities = []
    for i, ev in enumerate(multi):
        cand_data = []
        for m in ev["markets"]:
            try:
                tokens = json.loads(m.get("clobTokenIds") or "[]")
            ...
            if len(tokens) != 2 or outcomes != ["Yes", "No"]:
                continue
            yes_token = str(tokens[0]); no_token = str(tokens[1])
            # Get midpoint of YES side (proxy for price to buy YES)
            try:
                rr = requests.get(CLOB + "/midpoint", params={"token_id": yes_token},
                                  headers=UA, timeout=15)
                if not rr.ok:
                    continue
                data = rr.json()
                yes_mid = float(data.get("mid") or 0)
                if not (0 < yes_mid < 1):
                    continue
            except (requests.RequestException, ValueError, json.JSONDecodeError):
                continue
            time.sleep(SLEEP)
            # Try to get the actual YES ask price (what we'd pay to BUY YES)
            try:
                rr = requests.get(CLOB + "/price",
                    params={"token_id": yes_token, "side": "BUY"},
                    headers=UA, timeout=15)
                yes_ask = float(rr.json().get("price", yes_mid)) if rr.ok else yes_mid
            ...
            cand_data.append({
                ...
                "candidate": (m.get("groupItemTitle")
                              or m.get("question") or "?")[:40],
                "yes_mid": yes_mid, "yes_ask": yes_ask,
                "no_token": no_token,
                "volume": float(m.get("volumeNum") or 0),
            })
        if len(cand_data) < a.min_candidates:
            continue
        sum_yes_mid = sum(c["yes_mid"] for c in cand_data)
        sum_yes_ask = sum(c["yes_ask"] for c in cand_data)
        # No arb: sum(NO ask) = sum(1 - YES_bid) — but as a first pass, use YES asks
        # If sum(YES_ask) > 1.00, buying NO on all (at 1 - YES_bid, approximated as 1 - YES_ask) yields
        #   payout at resolution: (N-1) × $1 (all NOs win except winner's NO)
        #   cost: sum(1 - YES_ask) = N - sum(YES_ask)
        #   profit: (N-1) - (N - sum(YES_ask)) = sum(YES_ask) - 1
        # So profit per dollar depends on sum(YES_ask) - 1 vs cost N - sum(YES_ask)
        N = len(cand_data)
        no_cost = N - sum_yes_ask                        # total cost to buy 1 NO of each
        arb_profit = (N - 1) - no_cost                   # sum(YES_ask) - 1
        # Fee: geopolitics=0, else 5% multiplier
        fee_free = any(k in ev["event_title"].lower()
                       for k in ("iran","israel","ceasefire","conflict",
                                 "cartel","venezuela","russia","putin","china"))
        fee_mult = 0.0 if fee_free else 0.05
        avg_no_price = no_cost / N if N else 0
        fee_per_contract = fee_mult * avg_no_price * (1 - avg_no_price)
        total_fees = N * fee_per_contract
        net_profit = arb_profit - total_fees
        opportunities.append({
            "event_id": ev["event_id"], "event_title": ev["event_title"],
            "N_candidates": N, "sum_yes_mid": round(sum_yes_mid, 4),
            "sum_yes_ask": round(sum_yes_ask, 4),
            "no_cost_per_share_set": round(no_cost, 4),
            "gross_arb_profit_per_set": round(arb_profit, 4),
            "net_after_fees": round(net_profit, 4),
            "roi_pct_per_set": round(net_profit / no_cost * 100, 2) if no_cost > 0 else 0,
            "total_market_vol": ev["total_volume"],
            "fee_free": fee_free,
        })
        if (i + 1) % 5 == 0:
            print(f"  ...{i+1}/{len(multi)} events, {len(opportunities)} priced")

    if not opportunities:
        print("no priced opportunities"); return
    O = pd.DataFrame(opportunities).sort_values("net_after_fees", ascending=False)
    O.to_csv(a.out, index=False)
    print(f"\nsaved {len(O)} priced events -> {a.out}")

    arbs = O[O.net_after_fees > 0]
    print(f"\n=== ARBITRAGE OPPORTUNITIES: {len(arbs)} events with net profit > 0 ===")
    if len(arbs) == 0:
        print("no live arb right now. Market prices are consistent (sum(YES) <= 1).")
        print("Run this hourly during high-attention news events (elections, playoff decision days)")
        print("— those are when retail overpays for favorites and creates temporary arbs.")
    else:
        show_cols = ["event_title", "N_candidates", "sum_yes_ask",
                     "no_cost_per_share_set", "net_after_fees", "roi_pct_per_set",
                     "fee_free", "total_market_vol"]
        with pd.option_context("display.width", 260, "display.max_colwidth", 55):
            print(arbs[show_cols].round({"sum_yes_ask": 3, "no_cost_per_share_set": 3,
                                          "net_after_fees": 3, "roi_pct_per_set": 2,
                                          "total_market_vol": 0}).to_string(index=False))

    print(f"\n=== TOP 10 CLOSEST TO ARB (currently no profit but near breakeven) ===")
    near = O.sort_values("sum_yes_ask", ascending=False).head(10)
    with pd.option_context("display.width", 260, "display.max_colwidth", 55):
        show_cols = ["event_title", "N_candidates", "sum_yes_ask", "gross_arb_profit_per_set"]
        print(near[show_cols].round({"sum_yes_ask": 3, "gross_arb_profit_per_set": 3}).to_string(index=False))

    print("\nHOW TO EXECUTE (if opportunities found):")
    print("  1. Verify orderbook depth on each candidate's NO side at listed price")
    print("  2. Execute simultaneously — if any leg fails, unwind others fast")
    print("  3. Hold to resolution: N-1 shares pay $1, winner's NO pays $0")
    print("  4. Total profit = net_after_fees × number of share-sets bought")
    print("  5. Capital tied up = no_cost_per_share_set × sets, for time to resolution")

def cmd_poly_timing_clusters(a):
    """For target wallets: cluster realized round-trip PnL by week + known event windows.
    Shows which weeks/events produce most of their edge — so you know when to be
    attentive vs sit out."""
    import pandas as pd
    DATA = "https://data-api.polymarket.com"
    ...
    if a.wallets:
        ...
        wallets = []
        for n in ["Suge_White", "RicanDiablo", "kingsley1213"]:
            m = smart[smart["name"].str.lower() == n.lower()]
            if not m.empty:
                wallets.append(m.iloc[0]["wallet"].lower())
    if not wallets:
    ...
    # Known event windows — hand-curated for 2024-2026
    EVENT_WINDOWS = [
        ("2024-11-05", "2024-11-08", "US_election_2024"),
        ("2024-12-16", "2024-12-19", "Fed_Dec_2024"),
        ("2025-01-27", "2025-01-30", "Fed_Jan_2025"),
        ("2025-03-17", "2025-03-20", "Fed_Mar_2025"),
        ("2025-04-30", "2025-05-02", "Fed_May_2025"),
        ("2025-06-16", "2025-06-19", "Fed_Jun_2025"),
        ("2025-06-13", "2025-06-25", "Iran_Israel_war_Jun2025"),
        ("2025-07-29", "2025-08-01", "Fed_Jul_2025"),
        ("2025-09-16", "2025-09-18", "Fed_Sep_2025"),
        ("2025-10-28", "2025-10-30", "Fed_Oct_2025"),
        ("2025-12-08", "2025-12-11", "Fed_Dec_2025"),
        ("2026-01-27", "2026-01-29", "Fed_Jan_2026"),
        ("2026-03-16", "2026-03-19", "Fed_Mar_2026"),
        ("2026-04-27", "2026-04-30", "Fed_Apr_2026"),
        ("2026-06-15", "2026-06-18", "Fed_Jun_2026"),
        ("2026-06-24", "2026-07-04", "NYC_mayoral_primary"),
        ("2026-07-27", "2026-07-30", "Fed_Jul_2026"),
    ]
    EVENT_WINDOWS = [(pd.Timestamp(a_, tz='UTC'), pd.Timestamp(b_, tz='UTC'), n_)
                     for a_, b_, n_ in EVENT_WINDOWS]
        ...
        for _, r in df.sort_values("ts").iterrows():
            ...
            if side in ("BUY", "MINT"):
                ...
                while remaining > 0 and positions.get(asset):
                    ...
                    match = min(buy_size, remaining)
                    realized.append({
                        "wallet": nm, "entry_ts": buy_ts, "exit_ts": r["ts"],
                        "entry_price": buy_price, "exit_price": r["price"],
                        "title": buy_title,
                    })
                    remaining -= match; buy_size -= match
                    if buy_size <= 0: positions[asset].pop(0)
                    else: positions[asset][0] = (buy_ts, buy_size, buy_price, buy_title)
        all_rt.extend(realized)
    if not all_rt:
        print("no trades"); return
    rt = pd.DataFrame(all_rt)
    rt["topic"] = rt["title"].map(topic)
    def compute_pnl(row):
        fm = 0 if row.topic in FEE_FREE else FEE_MULT
        en = min(max(row.entry_price + SLIP, 0.01), 0.99)
    ...
    # Tag each round-trip with which event window it was entered in
    def in_which_event(ts):
        for start, end, name in EVENT_WINDOWS:
            if start <= ts <= end:
                return name
        return "OFF_EVENT"
    rt["event_window"] = rt["entry_ts"].map(in_which_event)
    rt["week"] = rt["entry_ts"].dt.to_period("W").dt.start_time
    rt["month"] = rt["entry_ts"].dt.to_period("M").dt.start_time

    print(f"\n=== TOTAL: n={len(rt)}  PnL=${rt.pnl.sum():+,.0f}  Sharpe={rt.pnl.mean()/rt.pnl.std():+.3f} ===")

    print(f"\n=== BY EVENT WINDOW ===")
    g = rt.groupby("event_window").agg(
        n=("pnl", "size"), hit=("pnl", lambda x: (x > 0).mean()),
        total_pnl=("pnl", "sum"), mean_pnl=("pnl", "mean"),
        sharpe=("pnl", lambda x: x.mean()/x.std() if x.std() > 0 else 0),
    ).reset_index().sort_values("total_pnl", ascending=False)
    print(g.round({"hit": 3, "mean_pnl": 2, "sharpe": 3, "total_pnl": 0}).to_string(index=False))

    print(f"\n=== TOP 15 WEEKS BY PnL ===")
    weekly = rt.groupby("week").agg(
        n=("pnl", "size"), total=("pnl", "sum"), hit=("pnl", lambda x: (x > 0).mean()),
    ).reset_index().sort_values("total", ascending=False).head(15)
    print(weekly.round({"hit": 3, "total": 0}).to_string(index=False))

    print(f"\n=== BOTTOM 10 WEEKS (biggest losses) ===")
    worst = rt.groupby("week").agg(
        n=("pnl", "size"), total=("pnl", "sum"),
    ).reset_index().sort_values("total").head(10)
    print(worst.round({"total": 0}).to_string(index=False))

    # Concentration analysis: what share of PnL comes from top-N weeks
    weekly_all = rt.groupby("week")["pnl"].sum().sort_values(ascending=False)
    total = weekly_all.sum()
    for n in [5, 10, 20, 50]:
        if len(weekly_all) >= n:
            share = weekly_all.head(n).sum() / total * 100
            print(f"top {n:2d} weeks = {share:.1f}% of total PnL")

    # Off-event vs on-event comparison
    on = rt[rt.event_window != "OFF_EVENT"]
    off = rt[rt.event_window == "OFF_EVENT"]
    print(f"\n=== ON vs OFF EVENT WINDOWS ===")
    for label, sub in [("ON_EVENT", on), ("OFF_EVENT", off)]:
        if len(sub):
            s = sub.pnl.std() or 1e-9
            print(f"  {label:10s}  n={len(sub):4d}  hit={(sub.pnl>0).mean():.1%}  "
                  f"total=${sub.pnl.sum():+,.0f}  Sharpe={sub.pnl.mean()/s:+.3f}")

    rt.to_csv(a.out, index=False)
    print(f"\nsaved -> {a.out}")

def cmd_poly_wallet_correlation(a):
    """Are your target wallets independent, or the same person / same signal source?
    For each pair: computes (a) % of markets both traded, (b) median time delta between
    their trades on shared markets, (c) side-agreement rate on same-market trades.
    High overlap + tight timing + same side = FAKE diversification."""
    import pandas as pd
    DATA = "https://data-api.polymarket.com"
    ...
    if a.wallets:
        ...
        wallets = []
        for n in ["Suge_White", "RicanDiablo", "kingsley1213", "Cmurrayy", "Slugagoal"]:
            m = smart[smart["name"].str.lower() == n.lower()]
            if not m.empty:
                wallets.append(m.iloc[0]["wallet"].lower())
    if len(wallets) < 2:
        print("need at least 2 wallets"); return
    name_map = dict(zip(smart["wallet"].str.lower(), smart["name"]))
    ...
    print(f"fetching trades for {len(wallets)} wallets...")
    wallet_trades = {}
    for w in wallets:
        nm = name_map.get(w, w[:10])
        print(f"  {nm}...")
        ...
        df["side_upper"] = df["side"].astype(str).str.upper() if "side" in df.columns else ""
        df = df.dropna(subset=["price", "size", "ts"])
        # BUY-only for correlation — entries define thesis, not exits
        df = df[df.side_upper.isin(["BUY", "MINT"])]
        df["market_side"] = df["conditionId"].astype(str) + "_" + df.get("outcome", "?").astype(str)
        wallet_trades[nm] = df
    ...
    # Per-wallet stats
    print(f"\n=== PER-WALLET TRADE UNIVERSE ===")
    for nm, df in wallet_trades.items():
        print(f"  {nm}: {len(df)} buys across {df['conditionId'].nunique()} unique markets, "
              f"date range {df.ts.min().date()} → {df.ts.max().date()}")

    # Pairwise correlation
    print(f"\n=== PAIRWISE OVERLAP ANALYSIS ===")
    pairs = []
    names = list(wallet_trades.keys())
    for i in range(len(names)):
        for j in range(i+1, len(names)):
            n1, n2 = names[i], names[j]
            d1, d2 = wallet_trades[n1], wallet_trades[n2]
            m1 = set(d1["conditionId"].astype(str))
            m2 = set(d2["conditionId"].astype(str))
            shared_mkts = m1 & m2
            if not shared_mkts:
                pairs.append(dict(pair=f"{n1} vs {n2}", shared_markets=0,
                                   pct_of_smaller=0, side_agreement=None,
                                   median_time_delta_hrs=None, flag="NO OVERLAP"))
                continue
            # Overlap percentage
            smaller = min(len(m1), len(m2))
            pct_shared = len(shared_mkts) / smaller * 100
            # Side agreement + time delta
            merged = d1[d1["conditionId"].astype(str).isin(shared_mkts)][["conditionId","outcome","ts"]].rename(
                columns={"outcome":"outcome_1","ts":"ts_1"}).merge(
                d2[d2["conditionId"].astype(str).isin(shared_mkts)][["conditionId","outcome","ts"]].rename(
                    columns={"outcome":"outcome_2","ts":"ts_2"}),
                on="conditionId", how="inner")
            merged["same_side"] = merged["outcome_1"] == merged["outcome_2"]
            merged["delta_hrs"] = (merged["ts_2"] - merged["ts_1"]).dt.total_seconds().abs() / 3600
            side_agree = merged["same_side"].mean() * 100 if len(merged) else None
            # For same-side trades, compute median time between entries
            same_side_pairs = merged[merged.same_side]
            med_delta = same_side_pairs["delta_hrs"].median() if len(same_side_pairs) else None
            # Flag: high overlap + high side agreement + tight timing = suspicious
            flag = ""
            if pct_shared > 40 and side_agree and side_agree > 75:
                if med_delta and med_delta < 2:
                    flag = "*** SAME ACCOUNT / TIGHT COORD ***"
                elif med_delta and med_delta < 24:
                    flag = "** COORDINATED / SAME SIGNAL SOURCE **"
                else:
                    flag = "* CORRELATED but wider timing *"
            elif pct_shared > 30 and side_agree and side_agree > 65:
                flag = "moderate overlap"
            pairs.append(dict(pair=f"{n1} vs {n2}", shared_markets=len(shared_mkts),
                               pct_of_smaller=round(pct_shared, 1),
                               side_agreement=round(side_agree, 1) if side_agree else None,
                               median_time_delta_hrs=round(med_delta, 2) if med_delta else None,
                               flag=flag))
    P = pd.DataFrame(pairs)
    with pd.option_context("display.width", 240, "display.max_colwidth", 45):
        print(P.to_string(index=False))
    P.to_csv(a.out, index=False)

    # Deep dive on shared-market timing distribution for the tightest pair
    tight = [p for p in pairs if p["median_time_delta_hrs"] is not None
             and p["median_time_delta_hrs"] < 24 and p["shared_markets"] >= 20]
    if tight:
        tight.sort(key=lambda x: x["median_time_delta_hrs"])
        p = tight[0]
        n1, n2 = p["pair"].split(" vs ")
        d1, d2 = wallet_trades[n1], wallet_trades[n2]
        shared = set(d1["conditionId"].astype(str)) & set(d2["conditionId"].astype(str))
        merged = d1[d1["conditionId"].astype(str).isin(shared)][["conditionId","outcome","ts","title"]].rename(
            columns={"outcome":"outcome_1","ts":"ts_1"}).merge(
            d2[d2["conditionId"].astype(str).isin(shared)][["conditionId","outcome","ts"]].rename(
                columns={"outcome":"outcome_2","ts":"ts_2"}),
            on="conditionId", how="inner")
        merged["same_side"] = merged["outcome_1"] == merged["outcome_2"]
        merged["delta_hrs"] = (merged["ts_2"] - merged["ts_1"]).dt.total_seconds() / 3600
        print(f"\n=== TIME DELTA DISTRIBUTION for {n1} vs {n2} (same-side entries only) ===")
        s = merged[merged.same_side]["delta_hrs"].abs()
        for pct in [5, 10, 25, 50, 75, 90]:
            print(f"  p{pct}: {s.quantile(pct/100):.2f} hours")
        print(f"  under 5 min:  {(s < 5/60).mean()*100:.1f}%  <- if >20%, they're bots on same signal")
        print(f"  under 1 hr:   {(s < 1).mean()*100:.1f}%")
        print(f"  under 1 day:  {(s < 24).mean()*100:.1f}%")

    print(f"\n{'='*74}")
    print("INTERPRETATION:")
    print("  SAME ACCOUNT (>75% side-agree + <2hr median delta on 40%+ shared markets):")
    print("    → your 3 wallets are probably 1 trader. Fake diversification. Adjust risk.")
    print("  COORDINATED / SAME SIGNAL (<24hr delta):")
    print("    → they may all read the same news account or Discord. Still fake diversification,")
    print("      but their entries could reflect a real repeatable signal you can co-piggyback.")
    print("  MODERATE OVERLAP (30-50% shared, 65-75% side agreement):")
    print("    → some correlation, some independence. Real but reduced diversification.")
    print("  LOW OVERLAP:")
    print("    → true independence. Diversification benefit is real. Risk model unchanged.")

def cmd_poly_persistence_scan(a):
    """Full methodology: 30d leaderboard universe → per-block Sharpe persistence
    → copyability filter (hold time, directional) → realistic 3c slippage.
    Saves qualified wallets with addresses for immediate deployment."""
    import pandas as pd, numpy as np
    DATA = "https://data-api.polymarket.com"

    print(f"pulling MONTHLY leaderboard, top {a.n_universe}...")
    rows, offset = [], 0
    while offset < a.n_universe:
        try:
            r = requests.get(DATA + "/v1/leaderboard",
                params=dict(category="overall", timePeriod="month",
                            orderBy="PNL", limit=50, offset=offset),
                headers=UA, timeout=25)
        except requests.RequestException: break
        if not r.ok: break
        ...
        rows.extend(batch); offset += 50
        time.sleep(SLEEP)
    wallets = [(r["proxyWallet"].lower(), r.get("userName") or "?")
               for r in rows if isinstance(r.get("proxyWallet"), str)
               and r["proxyWallet"].startswith("0x")]
    print(f"  got {len(wallets)} wallets")
    if not wallets: return

    def topic(q):
        q = (q or "").lower()
        if any(k in q for k in ("iran","israel","lebanon","hormuz","khamenei")): return "iran_israel"
        if any(k in q for k in ("cartel","venezuela","russia","putin","china","qatar","north korea","honduras")): return "geopol_other"
        if "shutdown" in q or "government" in q: return "US_policy"
        if "fed" in q or "powell" in q or "cpi" in q: return "fed"
        if any(k in q for k in ("bitcoin","btc","ether","crypto")): return "crypto"
        if any(k in q for k in ("mayoral","election","president","senate","mamdani")): return "election"
        if any(k in q for k in ("trump","biden","harris","vance")): return "trump_admin"
        if any(k in q for k in ("world cup","masters","nba","nfl","boxing")): return "sports"
        return "other"
    FEE_FREE = {"iran_israel","geopol_other","US_policy"}; FEE_MULT=0.05; SLIP=0.03

    def fetch_trades(w, max_t):
        rows, offset = [], 0
        while offset < max_t:
            try:
                r = requests.get(DATA + "/trades",
    ...
    def analyze(trades):
        ...
        df["ts"] = pd.to_datetime(df["timestamp"], unit="s", errors="coerce", utc=True)
        df["side_u"] = df["side"].astype(str).str.upper() if "side" in df.columns else ""
        df = df.dropna(subset=["price","size","ts"])
        if df.empty: return None

        buy_ratio = (df.side_u.isin(["BUY","MINT"])).mean()

        positions, realized = {}, []
        for _, r in df.sort_values("ts").iterrows():
            asset = r.get("asset"); side = r.side_u
            size = r["size"]
            if not asset or size <= 0: continue
            title = str(r.get("title") or "")[:70]
            if side in ("BUY","MINT"):
        ...
        if len(realized) < a.min_n: return None
        rt = pd.DataFrame(realized)
        rt["topic"] = rt["title"].map(topic)

        med_hold = rt.hold_hrs.median()
        if med_hold < a.min_hold_hrs: return None
        if 0.40 < buy_ratio < 0.60: return None                 # too balanced = MM
        ...
        def pnl(row):
            ...
            c = 100/(en+ef)
            return c*(ex-xf) - c*(en+ef)
        rt["pnl"] = rt.apply(pnl, axis=1)

        rt_sorted = rt.sort_values("exit_ts").reset_index(drop=True)
        blocks = np.array_split(rt_sorted, 4)
        block_sharpes = []
        for b in blocks:
            if len(b) < 5:
                continue
            m = b.pnl.mean(); s = b.pnl.std() or 1e-9
            block_sharpes.append(m/s)
        if len(block_sharpes) < 3: return None
        positive_blocks = sum(1 for sh in block_sharpes if sh > 0.1)
        if positive_blocks < a.min_positive_blocks: return None

        overall_sharpe = rt.pnl.mean() / (rt.pnl.std() or 1e-9)
        total_pnl = rt.pnl.sum()
        days = (rt.exit_ts.max() - rt.entry_ts.min()).total_seconds() / 86400
        return dict(
            n=len(rt), n_days=round(days,0),
            median_hold_hrs=round(med_hold,1),
            buy_ratio=round(buy_ratio,2),
            overall_sharpe=round(overall_sharpe,3),
            total_pnl=round(total_pnl,0),
            dollars_per_week=round(total_pnl/(days/7),1) if days>7 else None,
            positive_blocks=positive_blocks,
            block_sharpes=[round(s,3) for s in block_sharpes],
        )

    print(f"\nscanning {len(wallets)} wallets (persistence + copyability + 3c slippage)...")
    qualified = []
    for i, (w, nm) in enumerate(wallets):
        if (i+1) % 10 == 0:
            print(f"  {i+1}/{len(wallets)} scanned, {len(qualified)} qualified")
        trades = fetch_trades(w, a.max_trades)
        stats = analyze(trades)
        if stats:
            stats["wallet"] = w; stats["name"] = nm
            qualified.append(stats)

    if not qualified:
        print(f"\nNO WALLETS QUALIFIED. Try lower --min-n or --min-positive-blocks."); return
    df = pd.DataFrame(qualified).sort_values("overall_sharpe", ascending=False)
    df.to_csv(a.out, index=False)
    print(f"\n{'='*70}\nQUALIFIED: {len(df)}")
    print(f"filters: n>={a.min_n}, med_hold>={a.min_hold_hrs}h, directional, "
          f"{a.min_positive_blocks}+/4 blocks Sharpe>0.1")
    print("="*70)
    cols = ["name","n","n_days","median_hold_hrs","buy_ratio",
            "overall_sharpe","total_pnl","dollars_per_week","positive_blocks","block_sharpes"]
    with pd.option_context("display.width", 260, "display.max_colwidth", 20):
        print(df[cols].to_string(index=False))
    print(f"\nsaved with ADDRESSES -> {a.out}")
    if len(df) >= 3:
        print(f"\nDEPLOYABLE (top 5 by Sharpe):")
        for _, r in df.head(5).iterrows():
            print(f"  {r['name']:20s}  {r['wallet']}")
...
def main():
    ...
    cb.add_argument("--filter-topics", default=None,
                    help="comma-sep topic whitelist (e.g. 'crypto,election,iran_israel,trump_admin,fed')")
    cb.add_argument("--filter-price-min", type=float, default=0.0,
                    help="min entry price for the sub-edge filter")
    cb.add_argument("--filter-price-max", type=float, default=1.0,
                    help="max entry price for the sub-edge filter")
    ...
    ar = sub.add_parser("poly-arb-scan")
    ar.add_argument("--min-candidates", type=int, default=3,
                    help="minimum number of outcomes to consider")
    ar.add_argument("--min-volume", type=float, default=50000,
                    help="minimum total volume across the event's markets")
    ar.add_argument("--out", default="poly_arbs.csv")
    tc = sub.add_parser("poly-timing-clusters")
    tc.add_argument("--smart-file", default="smart_wallets_v3.csv")
    tc.add_argument("--wallets", default=None,
                    help="comma-sep names/addresses; default = Suge_White,RicanDiablo,kingsley1213")
    tc.add_argument("--max-trades", type=int, default=5000)
    tc.add_argument("--out", default="poly_timing_clusters.csv")
    wc = sub.add_parser("poly-wallet-correlation")
    wc.add_argument("--smart-file", default="smart_wallets_v3.csv")
    wc.add_argument("--wallets", default=None,
                    help="comma-sep names/addresses; default = top 5 from widen-net")
    wc.add_argument("--max-trades", type=int, default=5000)
    wc.add_argument("--out", default="poly_wallet_correlation.csv")
    ps = sub.add_parser("poly-persistence-scan")
    ps.add_argument("--n-universe", type=int, default=200)
    ps.add_argument("--min-n", type=int, default=100)
    ps.add_argument("--min-hold-hrs", type=float, default=4.0)
    ps.add_argument("--min-positive-blocks", type=int, default=3)
    ps.add_argument("--max-trades", type=int, default=3000)
    ps.add_argument("--out", default="persistent_wallets.csv")
    a = ap.parse_args()
    d = {"selftest": cmd_selftest, "discover": cmd_discover, "snapshot": cmd_snapshot,
     "backtest": cmd_backtest, "calibration-pull": cmd_calibration_pull,
     "poly-calibration": cmd_poly_calibration, "poly-smartmoney": cmd_poly_smartmoney,
     "poly-scan": cmd_poly_scan, "poly-backtest": cmd_poly_backtest,
     "poly-consistency": cmd_poly_consistency, "poly-follow": cmd_poly_follow,
     "poly-timing": cmd_poly_timing, "poly-recent": cmd_poly_recent,
     ...
     "poly-widen-net": cmd_poly_widen_net, "poly-arb-scan": cmd_poly_arb_scan,
     "poly-timing-clusters": cmd_poly_timing_clusters,
     "poly-wallet-correlation": cmd_poly_wallet_correlation,
     "poly-persistence-scan": cmd_poly_persistence_scan}
    d[a.cmd](a)
