...
|---|---|---|
| `FLAT_BET_DOLLARS` | **35** | 50->25 in #151 (2026-08-22); 25->35 on 2026-08-28 and **35->50 on 2026-09-01, both OVERRIDES of that row's own "not on a good week" bar** — see PART II; **50->35 in #242 (2026-09-04), mechanical not discretionary**, on the ratio rule `bet <= (cash - STOP_BALANCE)/33` after cash fell to $1,643.65. Two overrides of the same clause in five days; a third means deleting the clause, not overriding it. **THIS ROW READ $50 FROM 2026-09-04 TO 2026-09-10 while the code read 35 — the same defect the CHARTER opens with ("config table said the bet was $50; it has been $25 since #151"), recurring within two weeks of the audit that was meant to end it. #242 shipped the constant and every derived figure but not this table. A sizing commit is not done until PART I is edited in the same PR.** `grep -n '^FLAT_BET_DOLLARS' late_certainty_trader.py` |
...
| book depth | **56** = `max(ceil(contracts x 1.5), 25)` at $35 | dynamic since #166; the legacy `MIN_BOOK_DEPTH=60` is no longer a gate. Not in harness (needs live book). **56 -> 80 on 2026-09-01 and 80 -> 56 again on #242 (2026-09-04), both purely from the bet change — nobody chose either move. The Sep 1 warning that this is the one self-rescaling constant that can COST volume is now moot in that direction: the requirement is back where it was before the raise.** Reproduce: `python3 -c "import math; print(max(math.ceil(37*1.5),25))"` (37 contracts is the live fill count at $35 / ~91c) |
...
| `STOP_BALANCE` | **400** | emergency floor. **Now 2.37x worst drawdown, above the 1.5x target** — 52.1 losses at $35 on a $2,224.82 balance (2026-09-10 13:36 ET). History of the ratio: 1.32x at $35/$1,405 (Aug 28) -> 1.45x at $50/~$2,000 (Sep 1) -> 1.13x at $50/$1,643.65, which triggered #242 -> 1.62x at $35 on that same cash -> 2.37x today as the balance recovered. **The ratio rule cuts AND permits: the ceiling `(cash - STOP_BALANCE)/33` is $55.30 today, so $35 is now 37% BELOW what the rule allows. That is a live decision for Chris, not a drift to fix silently — the "not on a good week" bar in the bet row still governs any raise.** Growth remains **P&L, not a deposit** (confirmed 2026-09-01), so the revisit trigger has not fired and 400 stands. The floor is now 18.0% of balance vs the ~33% it was set at — designed behaviour under a growing account, but a progressively weaker brake. Re-check the RATIO on any deposit/withdrawal. `python3 scripts/kstat.py` reports headroom and ratio directly |
| `CONSEC_LOSS_LIMIT` | 9 | never fires in 68d either way |
| trailing-24h limit | **bet x 20** ($700 at $35) | emergency brake; denominated in bets so a sizing change cannot disarm it — it rescaled $1,000 -> $700 through #242 with no code change, which is the whole point of the bets denomination. Confirm against a live run: `gh run view --log <id> \| grep daily_loss_limit` prints `daily_loss_limit=$700`. Now 31% of balance, down from the 50% it held across the $35 and $50 changes, because the bet fell while the bankroll rose |
