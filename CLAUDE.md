...
|---|---|---|
| `FLAT_BET_DOLLARS` | **35** | 50->25 in #151 (2026-08-22); **25->35 on 2026-08-28 as an OVERRIDE of that row's own "not on a good week" bar** — see PART II |
...
| book depth | **56** = `max(ceil(contracts x 1.5), 25)` at $35 | dynamic since #166; the legacy `MIN_BOOK_DEPTH=60` is no longer a gate. Not in harness (needs live book) |
...
| `STOP_BALANCE` | **400** | emergency floor. **Now 1.32x worst drawdown, not 1.5x** — headroom fell 40->28.7 losses at $35 and the stop was deliberately NOT lowered to restore the ratio; revisit on a large deposit/withdrawal |
| `CONSEC_LOSS_LIMIT` | 9 | never fires in 68d either way |
| trailing-24h limit | **bet x 20** ($700 at $35) | emergency brake; denominated in bets so a sizing change cannot disarm it |
